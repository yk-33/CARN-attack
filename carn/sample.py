...
def attack(net, device, dataset, cfg, alpha, T):
    ...
    for step, (hr, lr, name) in enumerate(dataset):
        ...
        lr0 = lr.unsqueeze(0).to(device)
        sr0 = net(lr0, cfg.scale).detach().squeeze(0)
        lr = lr0.squeeze(0)

        lr_rand = torch.Tensor(np.random.uniform(-0.01, 0.01, lr.shape)).type_as(lr).to(device)
        lr = lr + lr_rand
        lr = lr.unsqueeze(0)
        ...
        for temT in range(T):
            ...
            net.zero_grad()
            loss = torch.sqrt(loss_fun(sr, sr0) * lr.numel())  # L2距离
            print(loss)
            loss.backward()
        ...
        sr_im_path = os.path.join(sr_dir, "{}".format(name.replace("LR", "SR")))
        hr_im_path = os.path.join(hr_dir, "{}".format(name))

        save_image(sr, sr_im_path)
        save_image(lr, hr_im_path)
...
def main(cfg):
    ...
    attack(net, device, dataset, cfg, 8/255, 50)
